...
function Map({ data, dataRealT, apiKey, setKey }) {
	...
	useEffect(() => {
		...
		if (rentedUnits === "Full") {
			...
			dataRealTFilter = dataRealT.filter(
	...
	const getIcon = (location) => {
		...
		return L.divIcon({
			...
			iconAnchor: [12, 41],
			popupAnchor: [1, -34],
			html: `<img src="${markerIcon}" height="24px" width="24px" class="${
				activeMarkerId !== null &&
				location.coordinate.lat === activeMarkerId.lat &&
				location.coordinate.lng === activeMarkerId.lng
					? "map_marker"
					: ""
			}" />`,
		})
	...
	const zoomToCoordinate = (name, lat, lng) => {
		...
		if (map) {
			...
			const dataRealTZoom = dataRealT.filter(
				(field) => field.coordinate.lat === lat && field.coordinate.lng === lng
			)
				...
				<div className="map_search">
					<div className="map_search_bloc">
						...
						<div className="mobile_search_map">
							<input
								...
								onChange={(e) => FilteredAdress(e.target.value)}
								ref={searchBarRef}
							/>
					...
					{filteredAdresses.length > 0 && (
						<div className="map_search_suggestion">
							{filteredAdresses.map((address, index) => (
								<p
									className="map_search_text"
									onClick={() => zoomToCoordinate(address.name, address.lat, address.lng)}
									key={index}
								>
									{address.name}
								</p>
						...
						<div>
							<div className="map_filter_components_radio_content">
								...
								<input
									...
									step="5"
									defaultValue="0"
									onChange={(e) => onSetYieldRent(e.target.value / 10)}
									ref={YieldRangeRef}
								/>
							</div>
						</div>
						...
						<div className="map_filter_components_radio">
							{rentStarted === null ? (
								...
								>
									<p>Entamé</p>
									<div className="map_filter_components_radio_checkbox">
										<div className={`map_filter_components_radio_checkbox_button checked`}></div>
									</div>
									<p>Non entamé</p>
								</div>
							) : (
								...
								>
									<p>Entamé</p>
									<div className="map_filter_components_radio_checkbox">
										<div className={`map_filter_components_radio_checkbox_button unchecked`}></div>
									</div>
									<p>Non entamé</p>
								</div>
							)}
				...
				<div className={`map_marker_details_bloc ${open ? "open" : ""}`} ref={markerDetailsRef}>
					{selectedLocation && (
						<div className="map_marker_details_components">
							<div className="map_carousel_close" onClick={() => ResetSearch()}>
								<img src={cross} alt="chevron" height={14} />
							</div>
							<Carousel
								img={selectedLocation[indexLocation].image}
								name={selectedLocation[indexLocation].name}
								selectedLocation={selectedLocation}
								onIndexLocation={handleIndex}
							/>
							<h3 className="map_marker_details_components_name">
								{selectedLocation[indexLocation].name}
							</h3>
							<div className="map_marker_details_components_boutons">
								<a
									href={selectedLocation[indexLocation].marketplaceLink}
									className="bouton_marker_details"
									target="_blank"
									rel="noreferrer"
								>
									RealT
								...
								<a
									href={`https://dashboard.realtoken.community/asset/${selectedLocation[indexLocation].contract}`}
									className="bouton_marker_details"
									target="_blank"
									rel="noreferrer"
								>
								...
								<a
									href={`https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${selectedLocation[indexLocation].coordinate.lat},${selectedLocation[indexLocation].coordinate.lng}&heading=210&pitch=10&fov=80`}
									className="bouton_marker_details"
									target="_blank"
									rel="noreferrer"
								>
							...
							<div className="map_marker_details_components_field">
								...
								<p className="map_marker_details_components_field_text">
									{selectedLocation[indexLocation].rentedUnits}/
									{selectedLocation[indexLocation].totalUnits} (
									{formatNumber(
										(selectedLocation[indexLocation].rentedUnits /
											selectedLocation[indexLocation].totalUnits) *
											100,
							...
							<div className="map_marker_details_components_field">
								...
								<p className="map_marker_details_components_field_text">
									{formatNumber(selectedLocation[indexLocation].annualPercentageYield, 2)} %
								</p>
							</div>
							<div className="map_marker_details_components_field">
								...
								<p className="map_marker_details_components_field_text">
									{formatNumber(
										selectedLocation[indexLocation].netRentWeekPerToken *
											selectedLocation[indexLocation].ownedToken,
										2
									)}{" "}
									$
								</p>
							...
							<div className="map_marker_details_components_field">
								...
								<p className="map_marker_details_components_field_text">
									{formatNumber(
										selectedLocation[indexLocation].netRentYearPerToken *
											selectedLocation[indexLocation].ownedToken,
										2
									)}{" "}
									$
							...
							<div className="map_marker_details_components_field">
								...
								<p className="map_marker_details_components_field_text">
									{selectedLocation[indexLocation].rentStartDate
										.getDate()
										.toString()
										.padStart(2, "0")}
									/
									{(selectedLocation[indexLocation].rentStartDate.getMonth() + 1)
										.toString()
										.padStart(2, "0")}
									/{selectedLocation[indexLocation].rentStartDate.getFullYear()}
								</p>
							</div>
							<div className="map_marker_details_components_field">
								<p className="map_marker_details_components_field_title">Type de location</p>
								<p className="map_marker_details_components_field_text">
									{formatString(selectedLocation[indexLocation].rentalType)}
								</p>
							</div>
							<div className="map_marker_details_components_field">
								<p className="map_marker_details_components_field_title">Investissement total</p>
								<p className="map_marker_details_components_field_text">
									{selectedLocation[indexLocation].totalInvestment} $
								</p>
							</div>
							<div className="map_marker_details_components_field">
								<p className="map_marker_details_components_field_title">Token</p>
								<p className="map_marker_details_components_field_text">
									{formatNumber(selectedLocation[indexLocation].ownedToken, 2)}/
									{selectedLocation[indexLocation].totalToken}{" "}
								</p>
							</div>
							<div className="map_marker_details_components_field">
								<p className="map_marker_details_components_field_title">Prix</p>
								<p className="map_marker_details_components_field_text">
									{selectedLocation[indexLocation].price} $
								</p>
							</div>
