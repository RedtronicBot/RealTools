	...
	useEffect(() => {
		...
		if (historyState !== undefined) {
			...
			const YieldObjFirst = {
				yield: parseFloat(
					(
						(historyState.history[closestIndex].values.netRentYear /
			...
			for (var i = 0; i < historyState.history.length; i++) {
				...
				if (historyState.history[i].values.tokenPrice !== undefined) {
					...
					const ValueObj = {
						tokenPrice:
							historyState.history[i].values.tokenPrice *
							data.filter((field) => field.token === contract)[0]?.value,
						date: `${dateObject.getDate().toString().padStart(2, "0")}/${(dateObject.getMonth() + 1)
							.toString()
							.padStart(2, "0")}/${dateObject.getFullYear()}`,
					}
			...
			if (arrayYield.length === 0) {
				...
				const YieldObj = {
					yield: parseFloat(
						(
							(historyState.history[0].values.netRentYear /
								historyState.history[0].values.totalInvestment) *
							100
						).toFixed(2)
					),
			...
			for (var j = 0; j <= Math.round(weeks); j++) {
				...
				rent +=
					parseFloat(arrayHistoryRent[index].rent) *
					data.filter((field) => field.token === contract)[0]?.value
				roi = (
					(rent / (arrayToken[0].tokenPrice * data.filter((field) => field.token === contract)[0]?.value)) *
					100
				).toFixed(2)
				const rentObj = {
			...
			const ValueObj = {
				tokenPrice:
					arrayToken[arrayToken.length - 1].tokenPrice *
					data.filter((field) => field.token === contract)[0]?.value,
				date: `${dateObjectRentCopy.getDate().toString().padStart(2, "0")}/${(dateObjectRentCopy.getMonth() + 1)
					.toString()
					.padStart(2, "0")}/${dateObjectRentCopy.getFullYear()}`,
