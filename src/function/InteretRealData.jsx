...
function InteretRealData(
	...
	useEffect(() => {
		...
		if (rentData.length > 0) {
			...
			for (var j = 0; j < parseInt(weeks); j++) {
				date = `${dateGraph.getDate().toString().padStart(2, "0")}/${(dateGraph.getMonth() + 1)
					.toString()
					.padStart(2, "0")}/${dateGraph.getFullYear()}`
				capitalProj += parseInt(investmentWeekReal)
				capitalReinvestProj =
					capitalReinvestProj * (1 + yieldRentProj / 100 / 52) + parseInt(investmentWeekReal)
	...
	return { interestDataProj, realData }
}

export default InteretRealData
