        ...
        for(var i = 0;i< weeks;i++) {
            ...
            filteredDataRealT.forEach(field=>{
                ...
                if(data !== undefined) {
                    ...
                    if(field.rentalType.trim().toLowerCase() === 'pre_construction' || (field.rentedUnits !== 0 && field.rentalType.trim().toLowerCase() !== 'pre_construction') || field.productType === "loan_income")
                    {
                        const rentStartDate = new Date(field.rentStartDate.date)
                        yieldInitial += (data.history[0].values.netRentYear/data.history[0].values.totalInvestment)*100
                        if(firstDateAfter >= rentStartDate) {
                            if(data.history[closestIndex].values.netRentYear === undefined || field.rentalType === 'pre_construction') {
                                
                                yieldActuel += (data.history[0].values.netRentYear/data.history[0].values.totalInvestment)*100
                            }
                            else {
                                yieldActuel += (data.history[closestIndex].values.netRentYear/data.history[0].values.totalInvestment)*100
                            }
                            
                        }
                        if(data.history[closestIndex].values.netRentYear === undefined || field.rentalType === 'pre_construction') {
                            yieldAnnuel += (data.history[0].values.netRentYear/data.history[0].values.totalInvestment)*100
                        }
                        else {
                            yieldAnnuel += (data.history[closestIndex].values.netRentYear/data.history[0].values.totalInvestment)*100   
                        }
                        for(var j = data.history.length-1;j >= 0;j--)
                        {  
                            if((data.history[j].values.rentedUnits !== undefined && data.history[j].values.rentedUnits === field.totalUnits && data.history[j].values.netRentYear !== undefined) || (data.history[j].values.rentedUnits === undefined && data.history[j].values.netRentYear !== undefined) || (field.rentalType === 'pre_construction' && data.history[j].values.netRentYear !== undefined) || (field.productType === "loan_income" && data.history[j].values.netRentYear !== undefined))
                            {
                                if(field.rentalType === 'pre_construction')
                                {
                                    yieldLocationPleine += (data.history[0].values.netRentYear/data.history[0].values.totalInvestment)*100
                                }
                                else
                                {
                                    yieldLocationPleine += (data.history[j].values.netRentYear/data.history[0].values.totalInvestment)*100
                                }
                                break 
                            }
                        }
            ...
            const yieldObj = {
                yieldInitial:(yieldInitial/locationBought.filter(field=>field.rentStartDate !== null).length).toFixed(2),
                yieldActuel:FilteredData.length === 0 ?null:(yieldActuel/FilteredData.length).toFixed(2),
                yieldAnnuel:(yieldAnnuel/locationBought.filter(field=>field.rentStartDate !== null).length).toFixed(2),
                yieldLocationPleine:(yieldLocationPleine/locationBought.filter(field=>field.rentStartDate !== null).length).toFixed(2),
