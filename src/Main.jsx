...
import building from "./images/icons/building-columns-solid.svg"
/*Pages*/
...
function Main() {
	const { data, dataRealT, key, setKey, valueRmm, history, rentData, dataRMM, tokenBought } = Data()
	...
	useEffect(() => {
		if (burgerRef.current) {
			...
			openBurger
				...
				: (burgerRef.current.style.transform = `translateX(-${positionInfo.width + 10}px)`)
		}
	}, [openBurger])
		...
		<div className="main">
			<div className="main_menu_burger">
				...
				<div className="menu_burger_content" ref={burgerRef}>
					...
					<div className="menu_burger_icons" onClick={() => onSetPage("RMM")}>
						<img src={building} width={32} alt="location_dot" />
						<p>RMM</p>
					</div>
				</div>
			</div>
			<div className="mobile_main">
				<div className="mobile_menu_burger">
					...
					<div className="mobile_menu_burger_icons" onClick={() => setPage("RMM")}>
						<img src={building} width={20} alt="location_dot" />
					</div>
			...
			{key !== "" ? (
				dataRealT.length > 0 ? (
					<>
						...
						{page === "Loyer" && (
							<Loyer data={data} dataRealT={dataRealT} setKey={setKey} rentData={rentData} />
						)}
						{page === "Vente" && (
							<HistoriqueVente
								dataRealT={dataRealT}
						...
						{page === "RMM" && <RMM dataRMM={dataRMM} setKey={setKey} apikey={key} />}
					</>
				) : (
